...
addtional classes, which we will discuss in <<building_bridge,Building the bridge
project>>.
...
<<building_bridge,Building the bridge project>>.
...
"snapshot". (See (<<output_products,Output products>>.)
* inWildFly: the generated WAR will be run in WildFly. Defaults to
"true". (See <<output_products,Output products>>.)
...
* We'll discuss `Greet_Server` below in <<using_war,Using the generated WAR>>
...
We have already discussed, in <<SSE,SSE>> and <<other_uses_Any,Other uses of google.protobuf.Any>>,
...
described in <<building_bridge,Building the bridge project>> can be used
to supply those that are not detected.

Also, we mentioned in <<building_bridge,Building the bridge project>>
...
Note that <<using_war,Using the generated WAR>> discusses a step
